...
for i=1:na_max
    for j=1:nb_max
        for k=1:nc_max
            x=x+1;
            sys_armax=armax(ide_data,[i,j,k,nk]);
            [~,sim1(x),~]=compare(sys_armax,ide_data); %Fit for identification data calculated and hold on sim1 variable.
            [~,sim2(x),~]=compare(sys_armax,val_data); %Fit for validation data calculated and hold on sim2 variable.
            if x>=2
...
sys_armax=armax(ide_data,[na,nb,nc,nk]); %System Identified According The Adjusted Parameters

%-----Plotting-----%
fprintf('na=%d nb=%d nc=%d nk=%d\n',na,nb,nc,nk);
subplot(2,2,1)
compare(sys_armax,ide_data);
subplot(2,2,2)
compare(sys_armax,val_data);
subplot(2,2,[3,4])
compare(sys_armax,all_data);
%-----Plotting-----%